...
void draw() {
  ...
  if ( abs(sokonKun.getVelocityY()) <= 30.0 ) {


    if (keyStatus.matches(".*q.*")) sokonKun.openKnees();
    if (keyStatus.matches(".*w.*")) sokonKun.closeKnees();
    if (keyStatus.matches(".*o.*")) sokonKun.closeFeet();
    if (keyStatus.matches(".*p.*")) sokonKun.openFeet();
  }
}
...
void keyPressed() {
  ...
  //  sokonKun.openFeet();
  //}
  if (key == 'q' && !keyStatus.matches(".*q.*")) keyStatus += "q";
  if (key == 'w' && !keyStatus.matches(".*w.*")) keyStatus += "w";
  if (key == 'o' && !keyStatus.matches(".*o.*")) keyStatus += "o";
  if (key == 'p' && !keyStatus.matches(".*p.*")) keyStatus += "p";
}

void keyReleased() {
  println(key);
  if (key == 'q') keyStatus = keyStatus.replace("q", "");
  if (key == 'w') keyStatus = keyStatus.replace("w", "");
  if (key == 'o') keyStatus = keyStatus.replace("o", "");
  if (key == 'p') keyStatus = keyStatus.replace("p", "");
}
